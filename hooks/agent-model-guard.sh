...
#
# Allowlist:
#   scout/worker/judge - models pinned in their agent definitions (see agents/)
...
case "$st" in
  scout|worker|judge|fork) exit 0 ;;
esac
if [ -z "$model" ]; then
  printf '%s' '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"agent-model-guard: this Agent call has no model and would silently inherit the main-thread model. Re-issue it per the routing table: judgment -> judge (opus pinned; tag sonnet for light judgment), mechanical execution -> worker, read-only recon -> scout. Inheriting the main-thread model on purpose requires explicit user approval."}}'
  exit 0
fi
exit 0
